...
sequenceDiagram
    ...
    Note over JOB: token → tpAnuncio → categoria → produto → ordem → pergunta
...
- **Sem webhook:** sincronização é **pull** periódica; perguntas incluídas via polling (`perguntasSave`). Mensagens pós-venda ainda não sincronizadas.
